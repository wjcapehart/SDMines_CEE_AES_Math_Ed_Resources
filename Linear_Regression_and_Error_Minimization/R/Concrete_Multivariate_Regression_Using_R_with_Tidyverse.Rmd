...
You can add custom labels and tities!  (https://www.nceas.ucsb.edu/~frazier/RSpatialGuides/colorPaletteCheatsheet.pdf)
...
### 5.x Plotting More Than One Value at a Time!
...
```{r}
  ...
  concrete_dependant = subset(x      = concrete_tidy,
    ...
    concrete_independant = subset(x      = concrete_tidy,
                              subset = (Parameter != "Slump") &
                                       (Parameter != "Flow")  &
                                       (Parameter != "Compressive_Strength_28dy")
                                     )                        
...
```
In the past, I've gotten good results with this but in this case, I think it's too messy.  How about leveraging a box whisker?  
...
```
 ...
 How about leveraging a "violin" plot?  A violin plot's width swells in areas with more observations and contracts with sparser data so it is like looking at a probability distribition.
...
What this plot shows above is that "fat" areas show a lot of data while skinny parts of the "violin" shows smaller plots.
...
This is like using the correlation command for the whole dataframe!
...
For this we use an extention to the graphics packate called [geom_smooth()](https://ggplot2.tidyverse.org/reference/geom_smooth.html)
...
## 6.0 Multivariate Linear Regression
...
Our formula now has multiple independant values but still folllows the same style of solution...
...
The fitted version is the dependant variable w/ a "_hat"" at the end
...
And finally we can plot our acutal vs modelled values.  (I'm adding a trend line)
...
## 7.0 Regression Quality Metrics
...
```
For a linear or multivariate regression the average of our residiuals (the difference between each observation and prediction) *should* be zero.

The root mean squared error (RMSE) is shown here.  It shouldn't be zero since the residuals are squared before summing them up.  We technically should use the standard error of the estmate, but RMSE remains a common error metric.   We can always do both.  The standard error of the estimate takes into account the degrees of freedom which which now includes all of the independant variables (p).  We can get the standard erorr of the estimate from our 
...
```{r}
ggplot(data = concrete_independant) +            # invoke graphics environment using a given dataframe
  ...
  ggtitle("Yeh Superplasticizer Tests",
          subtitle = "Concrete Components") + # Custom Title
  
    ylab(expression('Amount (kg m'^-3*")")) +   # x-label
...
```{r}
ggplot(data = concrete_dependant) +            # invoke graphics environment using a given dataframe
...
maxp = max(fitted.S_v_all$Compressive_Strength_28dy,fitted.S_v_all$Compressive_Strength_28dy_hat)
minp = min(fitted.S_v_all$Compressive_Strength_28dy,fitted.S_v_all$Compressive_Strength_28dy_hat)
...
ggplot(data = fitted.S_v_all) +           # invoke graphics environment using a given dataframe
  ...
  geom_smooth(method  = "lm",      # use a simple linar model
              formula = y ~ x,     # lm-style formula
              se      = FALSE,      # display Confidence Intervals
              level   = 0.95,      # Confidene Level to Map Out
              colour  = "red",     # regression line color
              fill    = "magenta", # fill for confidence limits
              size    = 0.5)  +    # line thickness
...
fitted.S_v_all2 = fitted.S_v_all

fitted.S_v_all2$Compressive_Strength_28dy_hat = fitted.S_v_all2$Compressive_Strength_28dy_hat + 5


maxp = max(fitted.S_v_all2$Compressive_Strength_28dy,fitted.S_v_all2$Compressive_Strength_28dy_hat)
minp = min(fitted.S_v_all2$Compressive_Strength_28dy,fitted.S_v_all2$Compressive_Strength_28dy_hat)

# Making a simple X-Y scatterplot and adding a regression to it

ggplot(data = fitted.S_v_all2) +           # invoke graphics environment using a given dataframe
  ...
  geom_smooth(method  = "lm",      # use a simple linar model
              ...
              level   = 0.95,      # Confidene Level to Map Out
              colour  = "blue",     # regression line color
              fill    = "cyan", # fill for confidence limits
              size    = 0.5)  +    # line thickness
...
```{r}
linear_model.S_v_cwf  <- lm(data    = concrete,                             # your data frame
                            formula = Compressive_Strength_28dy ~ Cement +  # your formula
                                                    Fly_Ash+        Water)  


fitted.S_v_cwf = get_regression_points(model = linear_model.S_v_cwf)


fitted.S_v_cwf2 = fitted.S_v_cwf

fitted.S_v_cwf2$Compressive_Strength_28dy_hat = fitted.S_v_cwf2$Compressive_Strength_28dy_hat + 10


linear_model.S_v_cw  <- lm(data    = concrete,                             # your data frame
                            formula = Compressive_Strength_28dy ~ Cement +  # your formula
                                                        Water)  


fitted.S_v_cw = get_regression_points(model = linear_model.S_v_cw)


fitted.S_v_cw2 = fitted.S_v_cw

fitted.S_v_cw2$Compressive_Strength_28dy_hat = fitted.S_v_cw2$Compressive_Strength_28dy_hat + 10




maxp = max(fitted.S_v_cwf$Compressive_Strength_28dy,fitted.S_v_cwf$Compressive_Strength_28dy_hat)
minp = min(fitted.S_v_cwf$Compressive_Strength_28dy,fitted.S_v_cwf$Compressive_Strength_28dy_hat)

# Making a simple X-Y scatterplot and adding a regression to it

ggplot(data = fitted.S_v_cwf) +           # invoke graphics environment using a given dataframe
  ...
  geom_point(colour="darkgreen") +  # plot points
  
  geom_smooth(method  = "lm",      # use a simple linar model
              formula = y ~ x,     # lm-style formula
              se      = FALSE,      # display Confidence Intervals
              level   = 0.95,      # Confidene Level to Map Out
              colour  = "green",     # regression line color
              fill    = "cyan", # fill for confidence limits
              size    = 0.5)  +    # line thickness
...
model_frames = data.frame( Observed = fitted.S_v_cwf$Compressive_Strength_28dy,
                           A  = fitted.S_v_cwf2$Compressive_Strength_28dy_hat,
                           B  = fitted.S_v_all$Compressive_Strength_28dy_hat,
                           C  = fitted.S_v_cw$Compressive_Strength_28dy_hat,
                           D  = fitted.S_v_cwf$Compressive_Strength_28dy_hat-2)




model_gather = gather(data  =    model_frames, # your data frame
                         key   = "Model", # column name for your former columns
                         value = "Modeled", # column name for your data
                         A, B, C, D ) # the list for the columns to "gather"


maxp = max(model_gather$Observed,model_gather$Modeled)
minp = min(model_gather$Observed,model_gather$Modeled)


ggplot(data = model_gather,
       mapping =   aes(x      = Observed,    # x-value
      y      = Modeled,
      color  = Model)) +  # y-value) +           # invoke graphics environment using a given dataframe
  ...
  geom_abline(slope     = 1,       # NEW: add a very simple line
   ...
   geom_point(size=1)+  facet_grid(cols=vars(Model))+
    xlim(minp,maxp) + 
  ylim(minp,maxp) + 
    coord_fixed(ratio = 1)  
```
...
strength =  data.frame(Modeled = fitted.S_v_all$Compressive_Strength_28dy,
                       Model="Observed")
...
ggplot(data = model_gather) + 
  ...
  theme_bw( ) +            
   ...
   ggtitle("Yeh Superplasticizer Tests",
          subtitle = "Model Probability Distributions") +   

  

     xlab("28-dy Compressive Strength at Failure (MPa)")+

     ylab("probability desity function, p(strength)")+
  

  scale_x_continuous(breaks       = seq(0, 80, by = 5),
                     minor_breaks = seq(-5, 80, 1),limits = c(0,75))+
geom_vline(xintercept = 17,  color= "darkgrey")+
geom_vline(xintercept = 28,  color= "darkgrey")+
geom_vline(xintercept = 70,  color= "darkgrey")+
  geom_vline(xintercept = 45,  color= "darkgrey")+

   geom_density() +
 geom_density(data = strength,color="black")
...
geom_vline(xintercept = 70,  color= "darkgrey")+
  geom_vline(xintercept = 45,  color= "darkgrey")+
   ...
   stat_ecdf(geom = "step") +
 stat_ecdf(data = strength,color="black",geom = "step")
